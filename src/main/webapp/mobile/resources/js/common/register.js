$(document).ready(function(){
	$.formValidator.initConfig({formid:"form1",onerror:function(msg){showConfirmModal("nonono");return false;},onsuccess:function(){
		
        if(!$("input[type='checkbox'][id='xieyi_check']").is(':checked')) {
            //showConfirmModal('请选择注册协议后再提交');return false;
        }
	...
	/*$("#username").formValidator({onshow:"用户名由大小写字母6-30位组成",onfocus:"用户名由大小写字母6-30位组成",oncorrect:"该用户名可以注册"})
		...
		buttons: $("#button"),
		error: function(){showConfirmModal('服务器没有返回数据，可能服务器忙，请重试');},
...
function sendMobileCode(obj, msg_type) {
    ...
    //var obj = $(obj);
    if(!/^1[0-9]\d{9}$/.test(mobile)) {showConfirmModal('请填写正确的手机号码');return false;}
    if(!$.trim(code)){showConfirmModal('请填写验证码');return false;}
    ...
    $.ajax({
        ...
        success:function(data) {
            if(data.code==-1) {
            	...
            	showConfirmModal('手机号码已存在,请更换手机号码');
            	return false;
            }
        }
    });
...
function tm_projectRegister() {
	showConfirmModal('请先注册账号并登陆之后再查看项目详情!');
