...
<div>
	<ul class="log-ul">
		<li class="log-header">
		<div class="row">
			<div class="col-xs-5">充值时间</div>
			<div class="col-xs-4">充值金额（元）</div>
		...
		<c:forEach items="${rechargeRecords}" var="record" varStatus="index">
			<li class="log-item">
				<div class="row">
					<div class="col-xs-5">
						<fmt:formatDate value="${record.dealTime }" pattern="yy-MMM-dd HH:mm:ss"/>
					</div>
					<div class="col-xs-4">${record.amount }</div>
					<div class="col-xs-3">${record.dealState }</div>
				</div>
			</li>
		</c:forEach>
		<li class="log-item">
