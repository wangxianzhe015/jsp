...
(function($){
	$( function() {
		...
		$("#updateBtn").click( function() {
			//检测有没有选择员工
				var projectId_checked = $("[name='news.newsId']:checked");
		...
		$("#deleteBtn").click( function() {
			...
			var employeeId_checked = $("[name='news.newsId']:checked");
			if (employeeId_checked.length <= 0) {
				$.prompt('尚未选择!');
			} else {
				var yesorno = $.prompt('确定删除选中的新闻?', {
					buttons : {
						"确定" : true,
						"取消" : false
					},
	...
	<body>
		<div class="grid">
			<form action="" method="post" name="result_form" id="result_form">
				<div class="grid_head">
					...
					<div class="head_right">
						<a href="javascript:void(0);" id="addBtn"
							data-url="<%=servletPath %>/portal/news/entryAdd.action">新增新闻</a>
						<a href="javascript:void(0);" id="updateBtn"
							data-url="<%=servletPath %>/portal/news/entryUpdate.action">编辑新闻</a>
						<a href="javascript:void(0);" id="deleteBtn"
							data-url="<%=servletPath %>/portal/news/remove.action">删除新闻</a>
				...
				<div class="grid_body">
					...
					<table>
						...
						<tbody>
							<c:forEach items="${newss}" var="news" varStatus="index">
								<tr>
									<td class="center">
										<input type="checkbox" name="news.newsId"
									...
									<td>
										...
										<fmt:formatDate value="${news.newsTime}" pattern="yyyy-MM-dd HH:mm:ss"/>
									</td>																	
								</tr>
							</c:forEach>
						</tbody>
			...
			<form action="<%=servletPath %>/portal/news/index.action" method="post" name="page_form" id="page_form">
				<div class="grid_foot">
					<div class="foot_left">
						当前第
						...
						条,跳转到
						<select class="gotoPage" data-formAction="<%=servletPath %>/portal/news/index.action" data-formId="page_form">
					...
					<div class="foot_right">
						...
						<c:choose>
							...
							<c:otherwise>
								<a
									href="javascript:firstPage('<%=servletPath %>/portal/news/index.action', null, null);">首页</a>
							</c:otherwise>
						</c:choose>
						<c:choose>
							<c:when test="${page.first==true}">
							...
							<c:otherwise>
								<a
									href="javascript:prevPage('<%=servletPath %>/portal/news/index.action', null, 'page_form');">上一页</a>
							</c:otherwise>
						</c:choose>
						<c:choose>
							...
							<c:otherwise>
								<a
									href="javascript:nextPage('<%=servletPath %>/portal/news/index.action', null, 'page_form');">下一页</a>
							</c:otherwise>
						</c:choose>
						...
						<c:choose>
							...
							<c:otherwise>
								<a
									href="javascript:lastPage('<%=servletPath %>/portal/news/index.action', null, 'page_form');">末页</a>
							</c:otherwise>
						</c:choose>
					</div>
				</div>
