...
(function($){
	...
	<body>
		<div class="grid">
				...
				<div class="grid_head">
					...
					<div class="head_right">
						<a href="javascript:void(0);" id="addBtn"
							data-url="<%=servletPath %>/portal/projectnotice/entryAdd.action">新增项目公告</a>
						<a href="javascript:void(0);" id="updateBtn"
							data-url="<%=servletPath %>/portal/projectnotice/entryUpdate.action">编辑项目公告</a>
						<a href="javascript:void(0);" id="deleteBtn"
							data-url="<%=servletPath %>/portal/projectnotice/remove.action">删除项目公告</a>
						...
						<tbody>
							<c:forEach items="${notices}" var="notice" varStatus="index">
								<tr>
									...
									<td>
										<c:out value="${notice.project.projectName}"></c:out>
									</td>
									<td>
									   
										<c:out value="${notice.project.financeAmount }"></c:out>
									</td>									
									<td>
										<c:out value="${notice.project.projectStateName}"></c:out>
									</td>

									<td>
									    <div style="width:400px; white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">
										<c:out value="${notice.profile}"></c:out>
										</div>
									</td>									
								</tr>
							</c:forEach>
			...
			<form action="<%=servletPath %>/portal/projectnotice/index.action" method="post" name="page_form" id="page_form">
				<div class="grid_foot">
					<div class="foot_left">
						当前第
						...
						条,跳转到
						<select class="gotoPage" data-formAction="<%=servletPath %>/portal/projectnotice/index.action" data-formId="page_form">
					...
					<div class="foot_right">
						...
						<c:choose>
							...
							<c:otherwise>
								<a
									href="javascript:firstPage('<%=servletPath %>/portal/projectnotice/index.action', null, null);">首页</a>
							</c:otherwise>
						</c:choose>
						<c:choose>
							<c:when test="${page.first==true}">
							...
							<c:otherwise>
								<a
									href="javascript:prevPage('<%=servletPath %>/portal/projectnotice/index.action', null, 'page_form');">上一页</a>
							</c:otherwise>
						</c:choose>
						<c:choose>
							...
							<c:otherwise>
								<a
									href="javascript:nextPage('<%=servletPath %>/portal/projectnotice/index.action', null, 'page_form');">下一页</a>
							</c:otherwise>
						</c:choose>
						...
						<c:choose>
							...
							<c:otherwise>
								<a
									href="javascript:lastPage('<%=servletPath %>/portal/projectnotice/index.action', null, 'page_form');">末页</a>
							</c:otherwise>
						</c:choose>
					</div>
				</div>
