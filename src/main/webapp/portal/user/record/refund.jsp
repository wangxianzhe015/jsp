			...
			<div class="col-23 bg_white">
				<div class="clear user_tit_box">
					...
					<div class="fr more_box">
						...
						<a href="<%=servletPath %>/portal/uc/refundRecord.action" class="now">退款记录</a> 
					...
					<div class="buylist_cont_01">
						<div class="border-c buylist_cont_01div">
							<div class="buylist_title">
								<h3>退款明细</h3>
								<form action="<%=servletPath %>/portal/uc/refundRecord.action" method="post">
							...
							<div class="buylist_table">
								<table>
									...
									<c:if test="${empty refundRecords }">
										<tr>										   
											<td colspan="9">
												<img src="<%=resourcePath %>/images/i-warning.png">你当前没有退款记录
											</td>
										</tr>
									</c:if>
									<c:if test="${!empty refundRecords }">
										<c:forEach items="${refundRecords}" var="record" varStatus="index">
											<tr>
												<td>
													<c:out value="${record.orderId}"></c:out>
												</td>
												<td>
												   <c:out value="${record.origOrderId}"></c:out>
												</td>
												<td>
												...
												<td>
													<c:out value="${record.fee }"/>
												</td>
												<td>
													<c:if test="${record.refundType=='BANKCARD' }">
													银行退款
													</c:if>
													<c:if test="${record.refundType=='ACCOUNT' }">
													账户退款
													</c:if>
												</td>														
												<td>
												    <c:out value="${record.cause}"></c:out>
												</td>	
												<td>
