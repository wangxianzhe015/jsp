        ...
        <div class="col-23 bg_white">
			<div class="clear  user_tit_box">
                ...
                <div class="fr more_box">
                	<a href="<%=servletPath %>/portal/uc/receiveMessages.action" class="now">收信箱</a>
                    <a href="<%=servletPath %>/portal/uc/sendMessages.action">发信箱</a>
            	...
            	<div class="audit_check">
	  						<c:if test="${not empty messages }">
		  						<c:forEach items="${messages}" var="message" varStatus="index">
				               		...
				               		<p>
			                   			    发信人： ${message.fromName }	<em><fmt:formatDate value="${message.sendTime}" pattern="yyyy-MM-dd HH:mm:ss"/></em>                                  
				                    </p>
			...
			<form action="<%=servletPath %>/portal/uc/receiveMessages.action" method="post" name="page_form" id="page_form">
				<div class="grid_foot">
					<div class="foot_left">
						...
						条,跳转到
						<select class="gotoPage" data-formAction="<%=servletPath %>/portal/uc/receiveMessages.action" data-formId="page_form">
					...
					<div class="foot_right">
						...
						<c:choose>
							...
							<c:otherwise>
								<a
									href="javascript:firstPage('<%=servletPath %>/portal/uc/receiveMessages.action', null, null);">首页</a>
							</c:otherwise>
						</c:choose>
						<c:choose>
							<c:when test="${page.first==true}">
							...
							<c:otherwise>
								<a
									href="javascript:prevPage('<%=servletPath %>/portal/uc/receiveMessages.action', null, 'page_form');">上一页</a>
							</c:otherwise>
						</c:choose>
						<c:choose>
							...
							<c:otherwise>
								<a
									href="javascript:nextPage('<%=servletPath %>/portal/portal/uc/receiveMessages.action', null, 'page_form');">下一页</a>
							</c:otherwise>
						</c:choose>
						...
						<c:choose>
							...
							<c:otherwise>
								<a
									href="javascript:lastPage('<%=servletPath %>/portal/uc/receiveMessages.action', null, 'page_form');">末页</a>
							</c:otherwise>
						</c:choose>
					</div>
				</div>
