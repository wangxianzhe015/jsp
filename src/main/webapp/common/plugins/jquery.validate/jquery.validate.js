 ...
 * http://docs.jquery.com/Plugins/Validation Dual licensed under the MIT and GPL
...
(function($) {
	...
	$.extend($.validator, {
		...
		prototype: {
			...
			// http://docs.jquery.com/Plugins/Validation/Validator/form验证表单
			form: function() {
				this.checkForm();
			...
			resetForm: function() {//获取所有可提示错误的el，ignore表示忽略它，不提示错误
				if ($.fn.resetForm) $(this.currentForm).resetForm();
				this.submitted = {};
				this.lastElement = null;
				this.prepareForm();
				this.hideErrors();
//				 alert(this.elements().length);// 漏掉同名称的其他元素
...
/**
 * 封装校验方法,前台提交有两种方式，一种是换成type=submit，可以省去js中的form提交函数；一种是使用a标签，但是要加上form提交
 * 
 * @param form
 * @param userSettings
 * @return
 */
function validateForm(form, userSettings) {
	// 自定义的默认规则
	var st = {
		errorPlacement: function(error, element) {
			element.parent().append(error); // 这个不是固定的，自定义错误显示的位置,error是错误信息元素,element是被校验的元素input,最好加上id属性给元素
		},
		 focusCleanup:  false,
		// 光标进来时，移除验证
		focusInvalid: false,
		...
		submitHandler: function(form) {
			form.submit(); // 表单提交在这里控制
		}
	};
	op = jQuery.extend({},st, userSettings);
	var vo = jQuery(form).validate(op);
	return vo;
}