...
@URI(description="订单支付管理",path="/mobile/order/*")
public class OrderPayManager extends WebAction{
	...
	@Action(description = "重新生成订单", responses = { 
			@Response(name = SUCCESS,forward=@Forward(url="/mobile/user/order/order_agreement.jsp")),
	...
	public String repay(String orderId){
		...
		if(StringUtils.isNullOrBlank(user.getRealName()) || StringUtils.isNullOrBlank(user.getIdentityCard())){
			...
			tips.setBackUrl(request.getContextPath()+"/platform/mobile/uc/safeSetting.action");
	...
	@Action(description = "导出PDF", responses = { 
			@Response(name = SUCCESS, forward=@Forward(url="/mobile/user/order/export_pdf.jsp")),
			@Response(name = FAILURE, forward=@Forward(url="/mobile/common/message_tips.jsp"))})
	...
	@Action(description = "进入支付确认", responses = { @Response(name = SUCCESS, forward=@Forward(url="/mobile/user/order/order_confirm.jsp")) })
	...
	@Action(description = "进入支付确认", responses = { @Response(name = SUCCESS, forward=@Forward(url="/mobile/user/order/order_pay.jsp")) })
	...
	@Action(description = "银行卡支付", responses = { @Response(name = SUCCESS, forward=@Forward(url="/mobile/common/message_tips.jsp"))})
	...
	@Action(description = "账户划款支付", responses = { @Response(name = SUCCESS, forward=@Forward(url="/mobile/common/message_tips.jsp")) })
