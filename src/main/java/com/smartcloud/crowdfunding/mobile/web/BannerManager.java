...
@URI(description="横幅管理",path="/mobile/banner/*")
public class BannerManager extends WebAction{

	@Reference
	private BannerService bannerService;
	
	@Action(description = "获取所有横幅", responses = { @Response(name = SUCCESS, forward=@Forward(url="/mobile/banner/banner_list.jsp")) })
	...
	@Action(description = "进入添加横幅", responses = { @Response(name = SUCCESS, forward=@Forward(url="/mobile/banner/banner_add.jsp")) })
	public String entryAdd(){
		
		return SUCCESS;
	}
	
	@Action(description = "添加横幅", responses = { @Response(name = SUCCESS, forward=@Forward(url="/mobile/banner/index.action")) })
	public String addBanner(Banner banner){
		bannerService.addBanner(banner);
		return SUCCESS;
	}
	
	@Action(description = "进入更新横幅", responses = { @Response(name = SUCCESS, forward=@Forward(url="/mobile/banner/banner_update.jsp")) })
	...
	@Action(description = "进入更新横幅", responses = { @Response(name = SUCCESS, forward=@Forward(url="/mobile/banner/index.action")) })
	public String updateBanner(Banner banner){
		this.bannerService.updateBanner(banner);
		return SUCCESS;
	}
	
	@Action(description = "删除横幅", responses = { @Response(name = SUCCESS, forward=@Forward(url="/mobile/banner/index.action")) })
