 ...
 *
 */
@URI(description="系统公告管理",path="/mobile/sysnotice/*",portal=true)
public class SystemNoticeManager extends WebAction{
	
	@Reference
	private SystemNoticeService noticeService;
	
	@Action(description = "系统公告列表", responses = { @Response(name = SUCCESS,forward=@Forward(url="/mobile/sysnotice/sysnotice_list.jsp")) })
	...
	@Action(description = "进入添加公告", responses = { @Response(name = SUCCESS, forward=@Forward(url="/mobile/sysnotice/sysnotice_add.jsp")) })
	public String entryAdd(){
		return SUCCESS;
	}
	
	@Action(description = "添加系统公告", responses = { @Response(name = SUCCESS, forward=@Forward(url="/mobile/sysnotice/index.action")) })
	public String addNotice(SystemNotice notice){
		noticeService.addSystemNotice(notice);
		return SUCCESS;
	}
	
	@Action(description = "进入更新公告", responses = { @Response(name = SUCCESS, forward=@Forward(url="/mobile/sysnotice/sysnotice_update.jsp")) })
	...
	@Action(description = "更新系统公告", responses = { @Response(name = SUCCESS, forward=@Forward(url="/mobile/sysnotice/index.action")) })
	public String updateNotice(SystemNotice notice){
		this.noticeService.updateSystemNotice(notice);
		return SUCCESS;
	}
	
	@Action(description = "删除系统公示", responses = { @Response(name = SUCCESS, forward=@Forward(url="/mobile/sysnotice/index.action")) })
