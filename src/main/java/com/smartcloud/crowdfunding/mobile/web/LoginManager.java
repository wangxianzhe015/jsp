 ...
 *
 */
@URI(description="登录管理",path="/mobile/login/*",portal=true)
public class LoginManager extends WebAction{
	...
	@Action(description = "登录", responses = { @Response(name = SUCCESS, forward=@Forward(url="/mobile/index/login.jsp")) })
	...
	@Action(description = "表单登录", responses = { @Response(name = SUCCESS, forward=@Forward(url="/mobile/common/message_tips.jsp")) })
		...
		if(user!=null){
			try {
				...
				if (right){
					...
					tips.setWaitSecond(3);
					tips.setBackUrl(request.getContextPath()+"/platform/mobile/main/index.action");
	...
	@Action(description = "注销", responses = { @Response(name = SUCCESS, forward=@Forward(url="/mobile/main/index.action")) })
