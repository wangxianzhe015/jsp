...
@URI(description="易宝支付管理",path="/portal/pay/*")
public class YeePayManager extends WebAction{
	...
	@Action(description = "查询银行卡绑定信息", responses = { @Response(name = SUCCESS, forward=@Forward(url="/portal/yeepay/query_card.jsp")) })
	...
	@Action(description = "查询订单信息", responses = { @Response(name = SUCCESS, forward=@Forward(url="/portal/yeepay/query_order.jsp")) })
	...
	@Action(description = "查询交易记录", responses = { @Response(name = SUCCESS, forward=@Forward(url="/portal/yeepay/query_trade.jsp")) })
	...
	@Action(description = "查询提现记录", responses = { @Response(name = SUCCESS, forward=@Forward(url="/portal/yeepay/query_withdraw.jsp")) })
	...
	@Action(description = "查询退款记录", responses = { @Response(name = SUCCESS, forward=@Forward(url="/portal/yeepay/query_refund.jsp")) })
	...
	@Action(description = "查询交易记录", responses = { @Response(name = SUCCESS, alert=@Alert(message="$message", url="/portal/yeepay/query_card.jsp")) })
