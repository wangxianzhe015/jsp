...
@URI(description="注册管理",portal=true,path="/portal/registry/*")
public class RegistryManager extends WebAction {
	...
	@Action(description = "密码找回", responses = { @Response(name = SUCCESS, forward=@Forward(url="/portal/password/passwordfind.jsp")) })
	...
	@Action(description = "重置密码", responses = { 
			@Response(name = SUCCESS, forward=@Forward(url="/portal/password/passwordreset.jsp")),
			@Response(name = FAILURE, forward=@Forward(url="/portal/password/passwordfind.jsp")),
			})
	...
	@Action(description = "密码重置成功", responses = { @Response(name = SUCCESS,forward=@Forward(url="/portal/common/message_tips.jsp")) })
	public String passwordSuccess(String code,String newPassword){
		...
		if(StringUtils.equals(code,resetPasswordVerifyCode)){
			...
			tips.setWaitSecond(3);
			tips.setBackUrl(request.getContextPath()+"/platform/portal/registry/findPassword.action");
	...
	@Action(description = "注册", responses = { @Response(name = SUCCESS, forward=@Forward(url="/portal/index/register.jsp")) })
