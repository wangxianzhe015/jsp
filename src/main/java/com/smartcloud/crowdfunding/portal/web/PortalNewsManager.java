 ...
 *
 */
@URI(description="网站新闻管理",path="/portal/news/*",portal=true)
public class PortalNewsManager extends WebAction{
	
	@Reference
	private PortalNewsService newsService;
	
	@Action(description = "系统公告列表", responses = { @Response(name = SUCCESS,forward=@Forward(url="/portal/news/news_list.jsp")) })
	...
	@Action(description = "进入添加公告", responses = { @Response(name = SUCCESS, forward=@Forward(url="/portal/news/news_add.jsp")) })
	public String entryAdd(){
		return SUCCESS;
	}
	
	@Action(description = "添加系统公告", responses = { @Response(name = SUCCESS, forward=@Forward(url="/portal/news/index.action")) })
	public String addNews(PortalNews news){
		newsService.addNews(news);
		return SUCCESS;
	}
	
	@Action(description = "进入更新公告", responses = { @Response(name = SUCCESS, forward=@Forward(url="/portal/news/news_update.jsp")) })
	public String entryUpdate(PortalNews news){
		response.addToRequest("news",newsService.getNews(news.getNewsId()));
		return SUCCESS;
	}
	
	@Action(description = "更新新闻", responses = { @Response(name = SUCCESS, forward=@Forward(url="/portal/news/index.action")) })
	public String updateNews(PortalNews news){
		this.newsService.updateNews(news);
		return SUCCESS;
	}
	
	@Action(description = "删除新闻", responses = { @Response(name = SUCCESS, forward=@Forward(url="/portal/news/index.action")) })
