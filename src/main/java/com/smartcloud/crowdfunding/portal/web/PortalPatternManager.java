...
@URI(description="合作伙伴管理",path="/portal/pattern/*")
public class PortalPatternManager extends WebAction{

	@Reference
	private PortalPatternService patternService;
	
	@Action(description = "获取所有合作伙伴", responses = { @Response(name = SUCCESS, forward=@Forward(url="/portal/pattern/pattern_list.jsp")) })
	...
	@Action(description = "进入新增合作伙伴", responses = { @Response(name = SUCCESS, forward=@Forward(url="/portal/pattern/pattern_add.jsp")) })
	public String entryAdd(){
		
		return SUCCESS;
	}
	
	@Action(description = "添加合作伙伴", responses = { @Response(name = SUCCESS, forward=@Forward(url="/portal/pattern/index.action")) })
	public String addPattern(PortalPattern pattern){
		patternService.addPortalPattern(pattern);
		return SUCCESS;
	}
	
	@Action(description = "进入更新合作伙伴", responses = { @Response(name = SUCCESS, forward=@Forward(url="/portal/pattern/pattern_update.jsp")) })
	...
	@Action(description = "进入更新合作伙伴", responses = { @Response(name = SUCCESS, forward=@Forward(url="/portal/pattern/index.action")) })
	public String updatePattern(PortalPattern pattern){
		this.patternService.updatePortalPattern(pattern);
		return SUCCESS;
	}
	
	@Action(description = "删除合作伙伴", responses = { @Response(name = SUCCESS, forward=@Forward(url="/portal/pattern/index.action")) })
